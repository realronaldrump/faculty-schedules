          ...
          return [
            {
              ...
              ...(item.locationType === "no_room" || item.isOnline
                ? { room: item.locationLabel || "No Room Needed" }
                : {}),
  ...
  const CustomDropdown = () => (
    <div className="relative">
      ...
      >
        ...
        <ChevronsUpDown
          className={`w-5 h-5 text-baylor-green transition-transform ${isDropdownOpen ? "transform rotate-180" : ""}`}
        />
      </button>
            ...
            {dayData.busyPeriods.length > 0 ? (
              <div className="space-y-3">
                {dayData.busyPeriods.map((period, index) => (
                  <div
                    key={index}
                    className="bg-baylor-gold/5 border border-baylor-gold/30 rounded-lg p-4 hover:bg-baylor-gold/10 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-semibold text-baylor-green">
                        {formatMinutesToTime(period.start)} -{" "}
                        {formatMinutesToTime(period.end)}
                      </div>
                      <div className="text-xs text-gray-500 bg-white rounded px-2 py-1">
                        {Math.floor((period.end - period.start) / 60)}h{" "}
                        {(period.end - period.start) % 60}m
                      </div>
                    </div>
            ...
            {dayData.availableSlots.length > 0 ? (
              <div className="space-y-3">
                {dayData.availableSlots.map((slot, index) => (
                  <div
                    key={index}
                    className="bg-baylor-green/5 border border-baylor-green/20 rounded-lg p-4 hover:bg-baylor-green/10 transition-colors"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-semibold text-baylor-green">
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">
          View detailed schedule and availability for any faculty member
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {Object.entries(dayNames).map(([dayCode, dayName]) => {
              const dayData =
                getIndividualAvailability(selectedIndividual)[dayCode];
              return (
                <DaySchedule
                  key={dayCode}
                  dayName={dayName}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {(() => {
                const availability =
                  getIndividualAvailability(selectedIndividual);
                const totalBusyPeriods = Object.values(availability).reduce(
                  (sum, day) => sum + day.busyPeriods.length,
                  0,
                );
                const totalAvailableSlots = Object.values(availability).reduce(
                  (sum, day) => sum + day.availableSlots.length,
                  0,
                );
                const totalTeachingHours = Object.values(availability).reduce(
                  (sum, day) => {
                    return (
                      sum +
                      day.busyPeriods.reduce(
                        (daySum, period) =>
                          daySum + (period.end - period.start) / 60,
                        0,
                      )
                    );
                  },
                ...
                return (
                  <>
                    ...
                    <div className="text-center p-4 bg-baylor-gold/10 rounded-lg">
                      <div className="text-2xl font-bold text-baylor-green">
                        {totalTeachingHours.toFixed(1)}h
                      </div>
                      <div className="text-sm text-gray-600">
                        Teaching Hours
                      </div>
      ...
      {!selectedIndividual && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600">
            Choose a faculty member from the dropdown above to view their
