...
const CourseManagement = ({ embedded = false }) => {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (filters.searchTerm && filters.searchTerm.trim() !== "") {
      ...
      data = data.filter((item) => {
        ...
        const matchesInstructor = instructorNames.some((name) =>
    ...
    if (
      filters.instructor &&
      Array.isArray(filters.instructor) &&
    ...
    if (filters.day && Array.isArray(filters.day) && filters.day.length > 0) {
      data = data.filter(
        (item) => item && item.Day && filters.day.includes(item.Day),
      );
    }

    if (
      filters.room &&
      Array.isArray(filters.room) &&
    ...
    if (
      filters.sections &&
      Array.isArray(filters.sections) &&
    ...
    if (
      ...
      data = data.filter((item) => {
        ...
        const normalizedProgram =
          rawProgram !== undefined && rawProgram !== null
            ? String(rawProgram).trim().toUpperCase()
            : "";

        const includeMatch =
          filters.programs.include.length === 0 ||
          filters.programs.include.includes(normalizedProgram);
        const excludeMatch =
          filters.programs.exclude.length === 0 ||
          !filters.programs.exclude.includes(normalizedProgram);
    ...
    if (
      ...
      const includeNormalized = filters.buildings.include.map((name) =>
        name.toUpperCase(),
      );
      const excludeNormalized = filters.buildings.exclude.map((name) =>
        name.toUpperCase(),
      );
      ...
      data = data.filter((item) => {
        ...
        const includeMatch =
          includeNormalized.length === 0 ||
          normalizedBuildings.some((building) =>
            includeNormalized.includes(building),
          );
        const excludeMatch =
          excludeNormalized.length === 0 ||
          !normalizedBuildings.some((building) =>
            excludeNormalized.includes(building),
          );
  ...
  const validateScheduleData = (data) => {
    ...
    const isOnline =
      data.isOnline === true || String(data.isOnline).toLowerCase() === "true";
    const hasDay =
      data.Day && typeof data.Day === "string" && /^([MTWRF]+)$/.test(data.Day);
    const hasTimes = Boolean(data["Start Time"]) && Boolean(data["End Time"]);
    const requiresMeetings =
      !isOnline ||
      (isOnline &&
        String(data.onlineMode || "").toLowerCase() === "synchronous");
    if (requiresMeetings) {
      if (!hasDay) {
        errors.push(
          "Valid day pattern is required (combination of M, T, W, R, F)",
        );
      }
      if (!hasTimes) {
    ...
    setEditFormData((prev) => {
      if (name === "Course") {
        ...
        return {
          ...
          Course: value,
          Credits:
            metadata.credits !== "" ? metadata.credits : (prev.Credits ?? ""),
  ...
  const handleAddCourse = () => {
    const dayPattern = Array.isArray(newCourseData.Day)
      ? newCourseData.Day.join("")
      : newCourseData.Day;
    ...
    const { credits, program, catalogNumber } = computeCourseMetadata(
      newCourseData.Course || "",
    );
    const courseWithId = {
      ...newCourseData,
      ...
      Room:
        Array.isArray(newCourseData.Rooms) && newCourseData.Rooms.length > 0
          ? newCourseData.Rooms.join("; ")
          : newCourseData.Room || "",
      id: `new_${Date.now()}`,
  ...
  const handleDownloadCSV = () => {
    if (selectedExportFields.length === 0) {
      setInlineError({
        context: "export",
        messages: ["Please select at least one field to export."],
      });
      return;
    }
    ...
    const stats = {
      ...
      uniqueInstructors: new Set(
        scheduleData.flatMap((s) => {
          if (!s) return [];
          if (Array.isArray(s.instructorNames) && s.instructorNames.length > 0)
            return s.instructorNames;
        ...
        const instructorIds =
          Array.isArray(s.instructorIds) && s.instructorIds.length > 0
            ...
            : s.instructorId
              ...
              : [];
        const instructorNames =
          Array.isArray(s.instructorNames) && s.instructorNames.length > 0
            ? s.instructorNames
            : s.Instructor
              ? [s.Instructor]
              : [];
    ...
    <div className="space-y-6">
      ...
      {!embedded && (
        <div>
          ...
          <p className="text-gray-600">
            View, edit, and manage course schedule information
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="text-2xl font-bold text-baylor-green">
            {courseStats.adjunctTaughtSessions}
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 border-b border-baylor-gold pb-4 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {filteredAndSortedData.length} of {scheduleData.length} courses
              shown
            </p>
          </div>
        ...
        {showAddCourseForm && (
          <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
            <h3 className="text-lg font-serif font-semibold text-baylor-green mb-3">
              Add New Course
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                <MultiSelectDropdown
                  options={uniqueRooms}
                  selected={
                    Array.isArray(newCourseData.Rooms)
                      ? newCourseData.Rooms
                      : []
                  }
                  onChange={(selected) =>
              ...
              <div>
                ...
                <div className="w-full p-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700">
                  {computedNewCourseCredits !== ""
                    ? computedNewCourseCredits
                    : "—"}
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  Auto-calculated from catalog number
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <div className="p-4 mb-6 bg-gray-50 rounded-lg border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <MultiSelectDropdown
              ...
              onChange={(selected) =>
                setFilters({ ...filters, instructor: selected })
              }
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
                <div>
                  ...
                  <MultiSelectDropdown
                    ...
                    onChange={(selected) =>
                      setFilters((prev) => ({ ...prev, sections: selected }))
                    }
          ...
          {activeFilterCount > 0 && (
            <div className="mt-4 pt-3 border-t border-gray-200">
              <div className="text-sm text-gray-600">
                ...
                of <span className="font-medium">{scheduleData.length}</span>{" "}
                courses shown
                {activeFilterCount > 0 && (
                  <span className="ml-2">
                  ...
                  >
                    <div className="flex-1">
                      <p className="font-medium text-gray-800">
                        {change.displayAction || change.action}:{" "}
                        {change.entity || "Unknown"}
                      </p>
                      {change.instructor && change.course && (
                        <p className="text-gray-600">
                          ...
                          's <span className="font-bold">{change.course}</span>{" "}
                          entry.
                        </p>
                      )}
                  ...
                  >
                    {editingRowId === row.id ? (
                      <>
                        ...
                        <td className="p-1">
                          <div className="w-full p-1 border border-baylor-gold rounded bg-gray-50 text-sm text-gray-700">
                            {(() => {
                              const metadata = computeCourseMetadata(
                                editFormData.Course || row.Course,
                              );
                      ...
                      <>
                        ...
                        <td className="px-4 py-3 text-gray-700">
                          {row.onlineMode
                            ? row.onlineMode.charAt(0).toUpperCase() +
                              row.onlineMode.slice(1)
                            : "-"}
                        </td>
                  ...
                  >
                    {scheduleData.length === 0 ? (
                      <div>
                        ...
                        <p className="text-sm">
                          Import schedule data to get started
                        </p>
                      </div>
                    ) : (
                      <div>
