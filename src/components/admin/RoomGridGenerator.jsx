...
const extractBuildingAndRoom = (roomString) => {
    ...
    const match = trimmed.match(/(.+?)\s+([\w\d\-\/]+)$/);
    if (match) {
        return {
...
const gatherRoomNames = (schedule) => {
    ...
    if (Array.isArray(schedule?.rooms)) {
        schedule.rooms.forEach((room) => addRoom(room?.displayName || room?.name));
    }
    addRoom(schedule?.roomName);
    addRoom(schedule?.room?.displayName || schedule?.room?.name);

    const rawRoom = schedule?.Room || schedule?.room;
    if (typeof rawRoom === 'string') {
        rawRoom.split(';').forEach(part => addRoom(part));
    }
...
const RoomGridGenerator = ({ scheduleData = [], rawScheduleData = [], selectedSemester = '', availableSemesters = [] }) => {
    ...
    useEffect(() => {
        if (dataSource === 'automatic' && selectedSemester && selectedSemester !== autoTerm) {
            setAutoTerm(selectedSemester);
        }
    }, [selectedSemester, dataSource, autoTerm]);
