...
const BaylorAcronyms = () => {
  ...
  const getAcronyms = useCallback(async () => {
    ...
    try {
      ...
      const acronymsData = data.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      }));
      const sortKey = (value) => (value ?? "").toString().toLowerCase();
      const sortedAcronyms = [...acronymsData].sort((a, b) => {
        const categoryCompare = sortKey(a.category).localeCompare(
          sortKey(b.category),
        );
      ...
      const uniqueCategories = [
        ...new Set(sortedAcronyms.map((item) => item.category)),
      ]
        .filter(Boolean)
        .sort();
      ...
      showNotification(
        "error",
        "Fetch Error",
        "Failed to load acronyms from the database.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAddAcronym = async () => {
    ...
    if (!newAcronym.acronym || !newAcronym.standsFor || !newAcronym.category) {
      showNotification(
        "error",
        "Validation Error",
        "Please fill out Acronym, Stands For, and Category.",
      );
      return;
    }
    ...
    const isDuplicate = acronyms.some(
      (acronym) =>
        acronym.acronym.trim().toLowerCase() ===
        newAcronym.acronym.trim().toLowerCase(),
    );
    ...
    if (isDuplicate) {
      showNotification(
        "error",
        "Duplicate Entry",
        `The acronym "${newAcronym.acronym}" already exists.`,
      );
      return;
    }
  ...
  const handleUpdateAcronym = async (id) => {
    ...
    const isDuplicate = acronyms.some(
      (acronym) =>
        acronym.id !== id &&
        acronym.acronym.trim().toLowerCase() ===
          editedAcronym.acronym.trim().toLowerCase(),
    );
    ...
    if (isDuplicate) {
      showNotification(
        "error",
        "Duplicate Entry",
        `The acronym "${editedAcronym.acronym}" already exists.`,
      );
      return;
    }
    ...
    try {
      ...
      // Optimistic update
      const updatedAcronyms = acronyms.map((acro) =>
        acro.id === id ? finalEditedAcronym : acro,
      );
      setAcronyms(updatedAcronyms);

      const updatedCategories = [
        ...new Set(updatedAcronyms.map((item) => item.category)),
      ]
        .filter(Boolean)
        .sort();
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-8 p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          <input
            ...
            placeholder="Acronym (e.g., HSD)"
            className="input-style col-span-1"
          />
          <input
            type="text"
            name="standsFor"
            value={newAcronym.standsFor}
            onChange={handleInputChange}
          ...
          <button
            ...
            className="btn-primary col-span-full lg:col-span-1"
          >
      ...
      {categories.length === 0 && !isLoading && (
        <div className="text-center py-12 px-6 bg-white rounded-lg border border-gray-200 shadow-sm">
          ...
          <p className="text-gray-500 mt-2">
            Get started by adding a new acronym using the form above.
          </p>
        </div>
      )}
      ...
      {categories.map((category) => (
        <div key={category} className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-3 p-3 bg-gray-100 rounded-lg border-l-4 border-baylor-green">
            {category}
          </h2>
                        ...
                        <>
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end space-x-2">
                              <button
                                onClick={() => handleUpdateAcronym(acronym.id)}
                                disabled={isSubmitting}
                          ...
                          <td className="px-4 py-3 text-right">
                            {confirmingDeleteId === acronym.id ? (
                              <div className="flex justify-end items-center space-x-2">
                                ...
                                <button
                                  onClick={() =>
                                    handleDeleteAcronym(acronym.id)
                                  }
                                  disabled={isSubmitting}
                              ...
                              <div className="flex justify-end space-x-2">
                                ...
                                <button
                                  onClick={() =>
                                    setConfirmingDeleteId(acronym.id)
                                  }
