...
const fetchDedupeDecisions = async (entityType) => {
 ...
 */
const findDuplicatePeople = async (options = {}) => {
 ...
 */
const backfillInstructorIdsFromNames = async () => {
 ...
 */
const standardizeAllData = async (options = {}) => {
  const {
    termCode = "",
    includePeople = true,
...
// A provided-but-empty scope means "scoped to nothing" (no candidates), while
// undefined/null means "unscoped" (all candidates). Conflating the two would
// let an import that touched zero records trigger cleanup across the full DB.
const hasScopedRecordIds = (scopedRecordIds) =>
  Array.isArray(scopedRecordIds) || scopedRecordIds instanceof Set;

const normalizeScopedRecordIds = (scopedRecordIds) => new Set(
  Array.from(scopedRecordIds || [])
    .map((id) => (id || "").toString().trim())
    .filter(Boolean),
);

export const filterDuplicatesByScopedRecordIds = (duplicates = [], scopedRecordIds) => {
  const safeDuplicates = Array.isArray(duplicates) ? duplicates : [];
  if (!hasScopedRecordIds(scopedRecordIds)) {
    return safeDuplicates;
  }

  const scopedIds = normalizeScopedRecordIds(scopedRecordIds);
  if (scopedIds.size === 0) {
    return [];
  }

  return safeDuplicates.filter((duplicate) => (
    (Array.isArray(duplicate?.records) ? duplicate.records : []).some((record) => (
      record?.id && scopedIds.has(String(record.id).trim())
    ))
  ));
};

export const runImportEntityResolutionCleanup = async ({
  transactionId = "",
  dryRun = false,
  ...
  peopleIds,
  roomIds,
  peopleMergeTypes = ["baylorId", "clssInstructorId", "email", "ignitePersonNumber"],
  roomMergeTypes = ["space_key", "room_name"],
  confidenceThreshold = {
    people: 1,
    rooms: 0.95,
  },
  ...
  const report = {
    mode: dryRun ? "preview" : "apply",
    transactionId: transactionId || "",
    peopleDuplicatesDetected: 0,
    peopleDuplicatesMerged: 0,
    peopleDuplicatesWouldMerge: 0,
    peopleDuplicatesFlagged: 0,
    peopleDuplicatesOutOfScope: 0,
    peopleMergeErrors: [],
    roomDuplicatesDetected: 0,
    roomDuplicatesMerged: 0,
    roomDuplicatesWouldMerge: 0,
    roomDuplicatesFlagged: 0,
    roomDuplicatesOutOfScope: 0,
    roomMergeErrors: [],
    scope: {
      peopleIds: hasScopedRecordIds(peopleIds)
        ? Array.from(normalizeScopedRecordIds(peopleIds))
        : null,
      roomIds: hasScopedRecordIds(roomIds)
        ? Array.from(normalizeScopedRecordIds(roomIds))
        : null,
    },
    mergeCandidates: {
      people: [],
  ...
  if (mergePeopleDuplicates) {
    const blockedPairs = await fetchDedupeDecisions("people");
    const peopleDuplicates = await findDuplicatePeople({ blockedPairs });
    const scopedPeopleDuplicates = filterDuplicatesByScopedRecordIds(
      peopleDuplicates,
      peopleIds,
    );
    report.peopleDuplicatesDetected = scopedPeopleDuplicates.length;
    report.peopleDuplicatesOutOfScope =
      peopleDuplicates.length - scopedPeopleDuplicates.length;
    const allowedTypes = new Set(peopleMergeTypes);

    for (const duplicate of scopedPeopleDuplicates) {
      const [primary, secondary] = duplicate.records || [];
      const canMerge =
        primary?.id &&
        secondary?.id &&
        allowedTypes.has(duplicate.type) &&
        duplicate.confidence >= confidenceThreshold.people;
      if (!canMerge) {
        report.peopleDuplicatesFlagged += 1;
        continue;
      }
      ...
      report.mergeCandidates.people.push(candidate);

      if (dryRun) {
        report.peopleDuplicatesWouldMerge += 1;
        continue;
      }

      try {
        await mergePeople(primary.id, secondary.id);
        report.peopleDuplicatesMerged += 1;
      } catch (error) {
        report.peopleMergeErrors.push(error?.message || String(error));
      }
    }
  }

  if (mergeRoomDuplicates) {
    const blockedPairs = await fetchDedupeDecisions("rooms");
    const roomsSnapshot = await getDocs(collection(db, "rooms"));
    const rooms = roomsSnapshot.docs.map((docSnap) => ({
      id: docSnap.id,
      ...docSnap.data(),
    }));
    const roomDuplicates = detectRoomDuplicates(rooms, { blockedPairs });
    const scopedRoomDuplicates = filterDuplicatesByScopedRecordIds(
      roomDuplicates,
      roomIds,
    );
    report.roomDuplicatesDetected = scopedRoomDuplicates.length;
    report.roomDuplicatesOutOfScope =
      roomDuplicates.length - scopedRoomDuplicates.length;
    const allowedTypes = new Set(roomMergeTypes);

    for (const duplicate of scopedRoomDuplicates) {
      const [primary, secondary] = duplicate.records || [];
      const canMerge =
        primary?.id &&
        secondary?.id &&
        allowedTypes.has(duplicate.type) &&
        duplicate.confidence >= confidenceThreshold.rooms;
      if (!canMerge) {
        report.roomDuplicatesFlagged += 1;
        continue;
      }
      ...
      report.mergeCandidates.rooms.push(candidate);

      if (dryRun) {
        report.roomDuplicatesWouldMerge += 1;
        continue;
      }

      try {
        await mergeRoomRecords(duplicate);
        report.roomDuplicatesMerged += 1;
      } catch (error) {
        report.roomMergeErrors.push(error?.message || String(error));
      }
    }
  }
...
const previewScheduleIdentityBackfill = async ({ termCode = "" } = {}) => {
...
const applyScheduleIdentityBackfill = async (changes = []) => {
...
const repairScheduleSpaceLinks = async ({
  normalizeRoomDisplayNames = true,
  overrides = {},
  schedules = null,
} = {}) => {
