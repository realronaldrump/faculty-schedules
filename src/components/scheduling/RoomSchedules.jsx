...
const RoomSchedules = ({ embedded = false }) => {
  ...
  // Computed derived state from selectedDate
  const roomScheduleDay = useMemo(() => {
    const jsDay = selectedDate.getDay(); // 0 Sun ... 6 Sat
    const mapping = { 1: "M", 2: "T", 3: "W", 4: "R", 5: "F" };
    return mapping[jsDay] || null; // Return null for weekends if not mapped
  }, [selectedDate]);

  const isDateInTerm = useMemo(() => {
    if (!selectedTermMeta?.startDate || !selectedTermMeta?.endDate) return true; // Assume valid if no dates set
  ...
  const dailyRoomSchedules = useMemo(() => {
    // If date is not in term, return empty (unless no dates defined for term)
    if (!isDateInTerm) return {};

    // If weekend or invalid day for scheduling
    if (!roomScheduleDay) return {};
    ...
    roomsToShow.forEach((room) => {
      schedules[room] = scheduleData
        ...
        .reduce((acc, item) => {
          ...
          if (
            !acc.some(
              (i) => `${i.Course}-${i["Start Time"]}-${i["End Time"]}` === key,
            )
          ) {
            acc.push(item);
          }
        ...
        schedules[room][day] = scheduleData
          ...
          .reduce((acc, item) => {
            ...
            if (
              !acc.some(
                (i) =>
                  `${i.Course}-${i["Start Time"]}-${i["End Time"]}` === key,
              )
            ) {
              acc.push(item);
            }
  ...
  const roomStats = useMemo(() => {
    ...
    if (Object.keys(dailyRoomSchedules).length === 0 && !isDateInTerm) {
      return {};
    }

    return stats;
  }, [dailyRoomSchedules, isDateInTerm]);

  // Compute final visible rooms considering toggles and sorting
  const visibleRooms = useMemo(() => {
    const baseRooms = Object.keys(dailyRoomSchedules);
    let roomsToProcess = baseRooms.length > 0 ? baseRooms : filteredRooms;

    if (showOnlyInUse) {
      roomsToProcess = roomsToProcess.filter(
        (r) => (dailyRoomSchedules[r] || []).length > 0,
      );
    }

    const sorted = [...roomsToProcess].sort((a, b) => {
      if (sortBy === "sessions") {
        return (
          (roomStats[b]?.sessions || 0) - (roomStats[a]?.sessions || 0) ||
          a.localeCompare(b, undefined, { numeric: true })
        );
      }
      if (sortBy === "utilization") {
        return (
          (roomStats[b]?.utilization || 0) - (roomStats[a]?.utilization || 0) ||
          a.localeCompare(b, undefined, { numeric: true })
        );
      }
    ...
    return sorted;
  }, [dailyRoomSchedules, roomStats, showOnlyInUse, sortBy, filteredRooms]);

  const visibleStats = useMemo(() => {
    const statsForVisible = visibleRooms
      .map((r) => roomStats[r])
      .filter(Boolean);
    const totals = statsForVisible.reduce(
      (acc, s) => {
    ...
    const avgUtil =
      statsForVisible.length > 0
        ? totals.utilizationSum / statsForVisible.length
        : 0;
    return {
  ...
  const handleExportWeekView = () => {
    ...
    const roomPart = selectedRoom
      ...
      : "all_rooms";
    const buildingPart = selectedBuilding
      ? `-${selectedBuilding.replace(/\s+/g, "_")}`
      : "";
    ...
    Object.entries(weeklyRoomSchedules).forEach(([room, daySchedules]) => {
      Object.entries(daySchedules).forEach(([day, schedules]) => {
        schedules.forEach((schedule) => {
          const meetingPattern = getMeetingPattern(
            schedule.Course,
            schedule["Start Time"],
            schedule["End Time"],
          );
      ...
      <div className="overflow-x-auto bg-white rounded-lg border border-gray-200">
        <div className="relative min-w-[800px]">
          ...
          {visibleRooms.map((room) => (
            <div
              key={room}
              className="relative flex items-center border-t border-gray-200 hover:bg-gray-50"
              style={{ height: rowHeight }}
            >
              <div className="w-40 flex-shrink-0 font-medium p-3 text-sm text-baylor-green border-r border-gray-200">
                <div className="font-semibold">{room}</div>
                <div className="text-xs text-gray-500">
                  {roomStats[room]?.sessions || 0} sessions •{" "}
                  {(roomStats[room]?.hours || 0).toFixed(1)}h
                </div>
                <div className="mt-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-baylor-green"
                    style={{
                      width: `${Math.min(100, Math.max(0, roomStats[room]?.utilization || 0))}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="absolute top-0 left-40 right-0 h-full bg-gray-50/30">
                ...
                {timeLabels.slice(1, -1).map((time) => (
                  <div
                    key={time}
                    style={{
                      left: `${((time - dayStart) / totalMinutes) * 100}%`,
                    }}
                ...
                {nowMinutes >= dayStart && nowMinutes <= dayEnd && (
                  <div
                    ...
                    style={{
                      left: `${((nowMinutes - dayStart) / totalMinutes) * 100}%`,
                    }}
                  />
                )}
          ...
          <div className="bg-baylor-green/5 px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-gray-600">
                {roomStats[room]?.sessions || 0} sessions •{" "}
                {(roomStats[room]?.hours || 0).toFixed(1)} hours •{" "}
                {(roomStats[room]?.utilization || 0).toFixed(0)}% utilization
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            {(dailyRoomSchedules[room] || []).length > 0 ? (
              <div className="space-y-3">
                {(dailyRoomSchedules[room] || []).map((session, index) => (
                  <div
                    key={index}
                    className={`flex items-center justify-between ${density === "compact" ? "p-3" : "p-4"} bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer`}
                    onClick={() => openCourseCard(session, room)}
                  >
            ...
            <div className="mt-4">
              ...
              <div className="mt-1 text-xs text-gray-500 text-right">
                {(roomStats[room]?.utilization || 0).toFixed(0)}% of day used
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-baylor-green focus:border-baylor-green"
                  data-tutorial="day-selector"
                />
                ...
                {!isDateInTerm && (
                  <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200 animate-fadeIn">
                    <Info className="w-4 h-4" />
                    <span>Outside of {selectedSemester} semester dates</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex-1 max-w-full">
              ...
              <div
                className="flex flex-wrap items-center gap-1 bg-gray-100 p-1 rounded-lg"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
            ...
            <div
              ...
              data-tutorial="only-in-use-toggle"
            >
              ...
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${showOnlyInUse ? "translate-x-5" : "translate-x-1"}`}
                />
              </button>
            </div>
      ...
      {viewMode !== "week" && (
        ...
        >
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div className="text-xs text-gray-500">
              on {selectedDate.toLocaleDateString("en-US", { weekday: "long" })}
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <div className="text-2xl font-bold text-baylor-green">
              {visibleStats.count > 0
                ? visibleStats.avgUtilization.toFixed(0)
                : 0}
              %
            </div>
