...
import { Search, Clock, Users, Calendar, X, ChevronDown, CheckCircle, ArrowUpDown, ChevronsUpDown, BarChart2 } from 'lucide-react';
...
const FacultyScheduleDashboard = () => {
  ...
  const [drillDownModalContent, setDrillDownModalContent] = useState({ title: '', data: [], columns: [] });
  ...
  // Effect to handle clicking outside modals to close them
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isRoomModalOpen && roomModalRef.current && !roomModalRef.current.contains(event.target)) {
        setIsRoomModalOpen(false);
      }
      if (isDrillDownModalOpen && drillDownModalRef.current && !drillDownModalRef.current.contains(event.target)) {
    ...
    const loadData = async () => {
      try {
        ...
        const data = lines.slice(1).flatMap(line => {
          ...
          if (rooms.length === 0) return [obj];
          return rooms.map(room => ({ ...obj, Room: room }));
        });
  ...
  const departmentInsights = useMemo(() => {
    ...
    const finalFacultyWorkload = Object.fromEntries(Object.entries(facultyWorkload).map(([instructor, data]) => [instructor, { courses: data.courseSet.size, totalHours: data.totalHours }]));
    return {
  ...
  // **NEW**: Memoized calculation for hourly usage based on the day filter
  const filteredHourCounts = useMemo(() => {
    ...
    const peakHour = Object.entries(hourCounts).reduce((max, [hour, count]) => count > max.count ? { hour: parseInt(hour), count } : max, { hour: 8, count: 0 });
  ...
  const sortedFacultyWorkload = useMemo(() => {
    ...
    return Object.entries(departmentInsights.facultyWorkload)
      .sort(([profA, dataA], [profB, dataB]) => {
        let valA, valB;
        if (key === 'name') { valA = profA; valB = profB; } 
        else { valA = dataA[key]; valB = dataB[key]; }
  ...
  const sortedRoomUtilization = useMemo(() => {
    ...
    return Object.entries(departmentInsights.roomUtilization)
      .sort(([roomA, dataA], [roomB, dataB]) => {
        let valA, valB;
        if (key === 'name') { valA = roomA; valB = roomB; } 
        else { valA = dataA[key]; valB = dataB[key]; }
  ...
  const handleDrillDown = (type, identifier, context) => {
    ...
    switch (type) {
      ...
      case 'roomList':
        ...
        columns = [{ key: 'name', label: 'Room Name' }];
        data = uniqueRooms.map(name => ({name}));
        break;
      case 'room':
        title = `Schedule for Room: ${identifier}`;
        ...
        data = scheduleData.filter(item => item.Room === identifier).sort((a,b) => (Object.keys(dayNames).indexOf(a.Day) - Object.keys(dayNames).indexOf(b.Day)) || (parseTime(a['Start Time']) - parseTime(b['Start Time'])));
        break;
      case 'hourly':
        const hour = parseInt(identifier);
        const dayFilter = context;
    ...
    setDrillDownModalContent({ title, data, columns });
        ...
        <div ref={roomModalRef} className="bg-white rounded-xl shadow-2xl p-6 max-w-2xl w-full mx-4">
          <div className="flex justify-between items-center mb-4 border-b border-baylor-gold pb-3">
            <div>
              ...
              <p className="text-md text-gray-700">For <span className="font-semibold">{dayName}</span>, from <span className="font-semibold">{formatMinutesToTime(meetingStart)}</span> to <span className="font-semibold">{formatMinutesToTime(meetingEnd)}</span></p>
            </div>
            <button onClick={() => setIsRoomModalOpen(false)} className="p-2 rounded-full hover:bg-gray-200"><X size={20} className="text-gray-600" /></button>
          </div>
          {availableRooms.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[60vh] overflow-y-auto pr-2">
              {availableRooms.map(room => (
                <div key={room} className="flex items-center p-3 bg-baylor-green/5 border border-baylor-green/20 rounded-lg"><CheckCircle className="w-5 h-5 text-baylor-green mr-3" /><span className="font-medium text-baylor-green">{room}</span></div>
              ))}
            </div>
          ) : (
             <div className="text-center py-12 text-gray-500"><div className="text-2xl mb-2">😔</div><p className="text-lg">No available rooms for this time slot.</p><p className="text-sm">Try a different time or a shorter meeting duration.</p></div>
          )}
          <div className="mt-6 text-right"><button onClick={() => setIsRoomModalOpen(false)} className={secondaryButtonClass}>Close</button></div>
        </div>
      </div>
    );
  };
  ...
  const renderDrillDownModal = () => {
    ...
    const { title, data, columns } = drillDownModalContent;
    return (
      <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in">
        <div ref={drillDownModalRef} className="bg-white rounded-xl shadow-2xl p-6 max-w-4xl w-full mx-4 flex flex-col" style={{maxHeight: '90vh'}}>
          <div className="flex justify-between items-center mb-4 border-b border-baylor-gold pb-3 flex-shrink-0">
            <h3 className="text-xl font-serif font-bold text-baylor-green">{title}</h3>
            <button onClick={() => setIsDrillDownModalOpen(false)} className="p-2 rounded-full hover:bg-gray-200"><X size={20} className="text-gray-600" /></button>
          </div>
          <div className="overflow-y-auto flex-grow">
            {data.length > 0 ? (
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-white shadow-sm">
                  <tr>{columns.map(col => <th key={col.key} className="px-4 py-3 text-left font-serif font-semibold text-baylor-green bg-baylor-green/5">{col.label}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {data.map((row, index) => (
                    <tr key={index} className="hover:bg-baylor-green/5">
              ...
              <div className="text-center py-12 text-gray-500"><p className="text-lg">No detailed data to display.</p></div>
            )}
          </div>
          <div className="mt-6 text-right flex-shrink-0"><button onClick={() => setIsDrillDownModalOpen(false)} className={secondaryButtonClass}>Close</button></div>
        </div>
      </div>
    );
  };
  ...
  if (loading || !departmentInsights) {
    return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-gray-600 text-xl">Loading faculty schedules...</div></div>;
  }
  ...
  return (
    <div>
      <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-6 border border-gray-200">
        <div className="flex">
          {[
            ...
            { id: 'insights', label: 'Department Insights', icon: BarChart2 },
          ].map(tab => {
            const Icon = tab.icon;
      ...
      <div className="space-y-6">
        {activeTab === 'group' && (
          <>{!showResults ? (<div><div className={cardClass}><div className="mb-8"><h2 className="text-xl font-serif font-semibold text-baylor-green mb-4 flex items-center border-b border-baylor-gold pb-2"><Clock className="mr-2 text-baylor-gold" size={20} />Step 1: Meeting Details</h2><div className="space-y-4"><div><label className="block text-sm font-medium text-gray-700 mb-2">Meeting Duration</label><div className="grid grid-cols-3 md:grid-cols-6 gap-3">{[30, 60, 90, 120, 150, 180].map(duration => <button key={duration} onClick={() => setMeetingDuration(duration)} className={`p-3 rounded-lg border text-center transition-all ${meetingDuration === duration ? 'bg-baylor-green text-white border-baylor-green shadow-md' : 'bg-gray-50 text-gray-700 border-gray-200 hover:bg-gray-100'}`}><div className="font-medium">{duration === 60 ? '1 hr' : duration === 120 ? '2 hrs' : `${duration}m`}</div></button>)}</div></div><div><label className="block text-sm font-medium text-gray-700 mb-2">Buffer Time (before and after)</label><div className="grid grid-cols-3 md:grid-cols-6 gap-3">{[0, 5, 10, 15, 20, 30].map(buffer => <button key={buffer} onClick={() => setBufferTime(buffer)} className={`p-3 rounded-lg border text-center transition-all ${bufferTime === buffer ? 'bg-baylor-gold text-baylor-green font-bold border-baylor-gold shadow-md' : 'bg-gray-50 text-gray-700 border-gray-200 hover:bg-gray-100'}`}><div className="font-medium">{buffer === 0 ? 'None' : `${buffer}m`}</div></button>)}</div></div></div></div><div className="mb-8"><h2 className="text-xl font-serif font-semibold text-baylor-green mb-4 flex items-center border-b border-baylor-gold pb-2"><Users className="mr-2 text-baylor-gold" size={20} />Step 2: Who needs to attend? ({selectedProfessors.length} selected)</h2><div className="mb-4"><div className="relative"><Search className="absolute left-3 top-3 text-baylor-green" size={16} /><input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className={inputClass} placeholder="Search professors..." /></div></div>{selectedProfessors.length > 0 && <div className="mb-4 p-4 bg-baylor-green/10 rounded-lg border border-baylor-green/20"><div className="flex flex-wrap gap-2">{selectedProfessors.map(professor => <span key={professor} className="inline-flex items-center px-3 py-1 bg-baylor-green text-white rounded-full text-sm">{professor}<button onClick={() => toggleProfessor(professor)} className="ml-2 hover:bg-baylor-green/80 rounded-full p-1"><X size={12} /></button></span>)}</div></div>}<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-64 overflow-y-auto">{filteredInstructors.map(professor => <button key={professor} onClick={() => toggleProfessor(professor)} className={`p-3 text-left rounded-lg border transition-all ${selectedProfessors.includes(professor) ? (professor === 'Staff' ? 'bg-baylor-gold/20 border-baylor-gold text-baylor-green' : 'bg-baylor-green/10 border-baylor-green text-baylor-green') : 'bg-white border-gray-200 hover:bg-gray-50'}`}><div className="flex items-center"><div className={`w-3 h-3 rounded-full mr-3 ${selectedProfessors.includes(professor) ? (professor === 'Staff' ? 'bg-baylor-gold' : 'bg-baylor-green') : 'bg-gray-300'}`}></div><span className="text-sm font-medium">{professor}{professor === 'Staff' && <span className="ml-2 text-xs bg-baylor-gold/20 text-baylor-green px-2 py-0.5 rounded-full">Staff</span>}</span></div></button>)}</div></div><div className="text-center"><button onClick={findMeetingTimes} disabled={selectedProfessors.length === 0} className={`${buttonClass} px-8 py-3 rounded-lg font-bold text-lg shadow-md`}><span className="flex items-center justify-center"><Calendar className="mr-2" size={18} />Find Available Times</span></button>{selectedProfessors.length === 0 && <p className="text-gray-500 text-sm mt-2">Select at least one professor to continue</p>}</div></div></div>) : (<div className="space-y-6"><div className={cardClass}><div className="flex items-center justify-between"><h2 className="text-xl font-serif font-semibold text-baylor-green mb-2">Meeting Times Found</h2><button onClick={() => setShowResults(false)} className={`${buttonClass} flex items-center`}><span className="mr-2">←</span> Back to Setup</button></div><div className="mt-4 p-3 bg-baylor-green/5 rounded-lg border border-baylor-green/20"><div className="flex flex-wrap gap-2">{selectedProfessors.map(professor => <span key={professor} className="px-3 py-1 bg-white rounded-lg text-sm text-baylor-green border border-baylor-green/30 font-medium">{professor}</span>)}</div></div></div><div className="grid gap-4">{Object.entries(dayNames).map(([dayCode, dayName]) => { const slots = commonAvailability[dayCode] || []; return (<div key={dayCode} className={cardClass}><div className="flex items-center justify-between mb-4"><h3 className="text-lg font-serif font-semibold text-baylor-green flex items-center"><Calendar className="mr-2 text-baylor-gold" size={18} />{dayName}</h3><span className={`px-3 py-1 rounded-full text-sm font-medium ${slots.length > 0 ? 'bg-baylor-green/10 text-baylor-green' : 'bg-red-100 text-red-800'}`}>{slots.length > 0 ? `${slots.length} slot${slots.length !== 1 ? 's' : ''}` : 'No availability'}</span></div>{slots.length > 0 ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{slots.map((slot, index) => <button key={index} onClick={() => handleSlotClick(dayCode, dayName, slot)} className="w-full text-left p-4 bg-baylor-green/5 border border-baylor-green/20 rounded-lg hover:bg-baylor-green/10 hover:shadow-md transition-all"><div className="font-semibold text-baylor-green text-lg">{formatMinutesToTime(slot.start)} - {formatMinutesToTime(slot.end)}</div><div className="text-sm text-baylor-green/80 mt-1">{Math.floor(slot.duration / 60)}h {slot.duration % 60}m window</div><div className="text-xs text-baylor-gold font-bold mt-2">Click to Find a Room →</div></button>)}</div> : <div className="text-center py-8 text-gray-500"><div className="text-lg">😞</div><div className="mt-2">No availability found for all participants</div></div>}</div>);})}</div></div>)}</>
        )}
        {activeTab === 'individual' && (<div className={cardClass}><h2 className="text-xl font-serif font-semibold text-baylor-green mb-6 border-b border-baylor-gold pb-2">Individual Professor Availability</h2><div className="mb-6"><label className="block text-sm font-medium text-gray-700 mb-2">Select Professor</label><CustomDropdown value={selectedIndividual} onChange={setSelectedIndividual} options={uniqueInstructors.map(i => ({ value: i, label: i }))} placeholder="Choose a professor..." className={selectClass} /></div>{selectedIndividual && <div className="space-y-4"><h3 className="text-lg font-serif font-semibold text-baylor-green border-b border-baylor-gold/50 pb-2">{selectedIndividual}'s Schedule & Availability</h3>{Object.entries(dayNames).map(([dayCode, dayName]) => { const dayData = getIndividualAvailability(selectedIndividual)[dayCode]; return (<div key={dayCode} className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm"><h4 className="font-serif font-semibold text-baylor-green mb-3">{dayName}</h4><div className="grid md:grid-cols-2 gap-4"><div><h5 className="text-sm font-medium text-baylor-green mb-2 border-b border-baylor-gold/30 pb-1">Classes & Commitments</h5>{dayData.busyPeriods.length > 0 ? <div className="space-y-2">{dayData.busyPeriods.map((p, i) => <div key={i} className="bg-baylor-gold/5 border border-baylor-gold/30 rounded-lg p-3"><div className="font-medium text-baylor-green">{formatMinutesToTime(p.start)} - {formatMinutesToTime(p.end)}</div><div className="text-sm text-baylor-green/80">{p.course} - {p.title}</div><div className="text-xs text-gray-500">{p.room}</div></div>)}</div> : <div className="text-gray-500 text-sm">No scheduled classes</div>}</div><div><h5 className="text-sm font-medium text-baylor-green mb-2 border-b border-baylor-gold/30 pb-1">Available Time Slots</h5>{dayData.availableSlots.length > 0 ? <div className="space-y-2">{dayData.availableSlots.map((s, i) => <div key={i} className="bg-baylor-green/5 border border-baylor-green/20 rounded-lg p-3"><div className="font-medium text-baylor-green">{formatMinutesToTime(s.start)} - {formatMinutesToTime(s.end)}</div><div className="text-sm text-baylor-green/80">{Math.floor(s.duration / 60)}h {s.duration % 60}m available</div></div>)}</div> : <div className="text-gray-500 text-sm">No gaps in schedule</div>}</div></div></div>); })}</div>}</div>
        )}
        {activeTab === 'rooms' && (<div className={cardClass}><h2 className="text-xl font-serif font-semibold text-baylor-green mb-6 border-b border-baylor-gold pb-2">Room Availability Finder</h2><div className="grid md:grid-cols-2 gap-6 mb-6"><div><label className="block text-sm font-medium text-gray-700 mb-2">Day</label><CustomDropdown value={roomSearchDay} onChange={setRoomSearchDay} options={Object.entries(dayNames).map(([c, n]) => ({ value: c, label: n }))} placeholder="Select day..." className={selectClass} /></div><div><label className="block text-sm font-medium text-gray-700 mb-2">Time</label><div className="relative"><input type="time" value={roomSearchTime} onChange={(e) => setRoomSearchTime(e.target.value)} className={timeInputClass} min="08:00" max="17:00" step="1800" /><div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none"><Clock className="w-4 h-4 text-baylor-green" /></div></div></div></div>{roomSearchDay && roomSearchTime && <div><h3 className="text-lg font-serif font-semibold text-baylor-green mb-4 border-b border-baylor-gold/30 pb-2">Available Rooms - {dayNames[roomSearchDay]} at {roomSearchTime}</h3>{(() => { const availableRooms = uniqueRooms.filter(room => !scheduleData.some(item => item.Day === roomSearchDay && item.Room === room && parseTime(roomSearchTime) >= parseTime(item['Start Time']) && parseTime(roomSearchTime) < parseTime(item['End Time']))); return (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{availableRooms.length > 0 ? availableRooms.map(room => <div key={room} className="bg-baylor-green/5 border border-baylor-green/20 rounded-lg p-4 hover:bg-baylor-green/10 transition-colors"><div className="font-medium text-baylor-green">{room}</div><div className="text-sm text-baylor-green/80 mt-1">Available</div></div>) : <div className="col-span-full text-center py-8 text-gray-500"><div className="text-lg">🚫</div><div className="mt-2">No rooms available at this time</div></div>}</div>);})()}</div>}</div>
        )}
        {activeTab === 'insights' && (
          <div className="space-y-6">
            ...
            <div className={`${cardClass} mb-0`}>
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 border-b border-baylor-gold/30 pb-2 gap-4">
                <div>
                  ...
                  <span className="text-sm font-normal text-baylor-green/80">Showing until {formatMinutesToTime(filteredHourCounts.latestEndTime)}</span>
                </div>
                <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg">
                    {['All', 'M', 'T', 'W', 'R', 'F'].map(day => (
                        <button key={day} onClick={() => setHourlyUsageDayFilter(day)} className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${hourlyUsageDayFilter === day ? 'bg-baylor-green text-white shadow' : 'text-gray-600 hover:bg-gray-200'}`}>
            ...
            <div className={`${cardClass} mb-0`}>
              <div className="flex justify-between items-center mb-4 border-b border-baylor-gold/30 pb-2">
                  ...
                  <div className="flex items-center gap-2">
                      <CustomDropdown value={roomSort.key} onChange={(key) => setRoomSort({ ...roomSort, key })} options={[{value: 'name', label: 'Sort by Name'}, {value: 'hours', label: 'Sort by Busiest'}, {value: 'classes', label: 'Sort by Sessions'}]} className="text-sm p-1 border border-gray-300 rounded-md bg-white"/>
                      <button onClick={() => setRoomSort(s => ({...s, direction: s.direction === 'asc' ? 'desc' : 'asc'}))} className="p-1.5 border border-gray-300 rounded-md hover:bg-gray-100"><ArrowUpDown size={14} className="text-gray-600" /></button>
                  </div>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-baylor-green/10">
                    {sortedFacultyWorkload.map(([instructor, data]) => (
                      <tr key={instructor} onClick={() => handleDrillDown('faculty', instructor)} className="hover:bg-baylor-green/5 transition-colors cursor-pointer">
                        <td className="px-4 py-3 text-sm text-baylor-green font-medium">{instructor}</td>
                        <td className="px-4 py-3 text-sm text-baylor-green/80 text-center">{data.courses}</td>
                        <td className="px-4 py-3 text-sm text-baylor-green/80 font-bold text-center">{data.totalHours.toFixed(1)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
