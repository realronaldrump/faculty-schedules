...
> Archived after the implementation was completed. The task checkboxes below
> preserve the source plan as it existed at archive time; the current review and
> verification record lives in `tasks/todo.md`.
...
## Review
- Canonical `programs.directors` assignments replaced person-side UPD flags and legacy program ID fields across reads, writes, filters, exports, and badges.
- The Data Cleanup migration previews merged/orphaned/manual-review outcomes before applying chunked canonical updates and legacy-field removal.
- Person delete/merge operations keep director assignments consistent, and Firestore rules validate the canonical shape while rejecting legacy fields.
- Model, migration, cross-view consistency, cleanup, export, lint, test, and production-build coverage were completed before this plan was archived.
