...
const StudentSchedules = ({ embedded = false }) => {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex rounded-md border">
              <button
                ...
                className={`px-3 py-1 text-xs rounded-l-md flex items-center gap-1 ${viewMode === "calendar"
              ...
              <button
                ...
                className={`px-3 py-1 text-xs rounded-r-md flex items-center gap-1 ${viewMode === "list"
                    ? "bg-baylor-green text-white"
                    : "bg-white text-gray-700 hover:bg-gray-50"
                  }`}
      ...
      {viewMode === "calendar" && (
        <div
          className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm overflow-x-auto"
          ref={calendarRef}
        >
          {/* Time scale header */}
          <div
            className="grid gap-x-2 min-w-max"
          ...
          >
            ...
            {visibleDays.map((d) => (
              <div
                key={d}
                className="text-center text-sm font-serif font-semibold text-baylor-green py-1"
              >
                {DAY_LABELS[d]}
              </div>
            ))}
          ...
          >
            ...
            <div
              className="relative"
              style={{ height: `${(totalMinutes / 60) * calendarPxPerHour}px` }}
            >
              {Array.from({ length: Math.floor(totalMinutes / 60) + 1 }).map(
                (_, i) => {
            ...
            {visibleDays.map((d) => (
              <div
                ...
                style={{
                  height: `${(totalMinutes / 60) * calendarPxPerHour}px`,
                }}
              >
                {/* Hour lines */}
                {Array.from({ length: Math.floor(totalMinutes / 60) + 1 }).map(
                ...
                {Array.from({ length: Math.floor(totalMinutes / 30) }).map(
                  (_, i) => {
                ...
                {(entriesByDayWithLayout[d] || []).map((item, idx) => {
                  ...
                  const eventHeightPx =
                    (durationMinutes / 60) * calendarPxPerHour;
                  let fontSizePx = 12;
