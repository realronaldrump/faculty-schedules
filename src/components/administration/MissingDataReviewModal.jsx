...
const MissingDataReviewModal = ({
  ...
  const loadMissingDataRecords = async () => {
    ...
    try {
      ...
      switch (missingDataType) {
        ...
        case "jobTitle":
          missingRecords = people.filter(
            (person) => !person.jobTitle || person.jobTitle.trim() === "",
          );
          break;
        case "program":
      ...
      setSaveResults({
        success: false,
        message: "Failed to load records: " + error.message,
      });
    }
    setIsLoading(false);
  };
  ...
  const saveRecord = async () => {
    ...
    try {
      ...
      setRecords((prev) =>
        prev.map((record) =>
          record.id === editingRecord.id ? { ...record, ...updates } : record,
        ),
      );
  ...
  const getFieldIcon = (type) => {
    switch (type) {
      ...
      case "program":
        return <BookUser className="w-4 h-4" />;
      default:
        return <User className="w-4 h-4" />;
    }
  };
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">
                ...
                {getFieldLabel(missingDataType).toLowerCase()}.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="mb-4">
                <p className="text-gray-600">
                  Found <span className="font-medium">{records.length}</span>{" "}
                  records missing {getFieldLabel(missingDataType).toLowerCase()}
              ...
              {records.map((record) => {
                ...
                const officeLabel =
                  record.hasNoOffice || studentWorker
                    ? "No office"
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">
                          Email: {record.email || "Missing"} • Phone:{" "}
                          {record.hasNoPhone
                            ? "No phone"
                            : record.phone || "Missing"}{" "}
                          • Office: {officeLabel}
                        </p>
                      </div>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className={`w-full px-3 py-2 border rounded-lg ${
                                  editingRecord.newEmail &&
                                  !validateEmail(editingRecord.newEmail)
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className={`flex-1 px-3 py-2 border rounded-lg ${
                                    editingRecord.newPhone &&
                                    !validatePhone(editingRecord.newPhone)
                                      ? "border-red-300 focus:border-red-500"
                                      : "border-gray-300 focus:border-baylor-green"
                                ...
                                <button
                                  ...
                                  title={
                                    editingRecord.newHasNoPhone
                                      ? "Mark as having no phone"
                                      : "Mark as having phone"
                                  }
                                >
                                  {editingRecord.newHasNoPhone ? (
                                    <PhoneOff size={16} />
                              ...
                              {editingRecord.newHasNoPhone && (
                                <p className="text-gray-600 text-xs mt-1">
                                  This person is marked as not having a phone
                                  number
                                </p>
                              )}
                            </div>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  title={
                                    editingRecord.newHasNoOffice
                                      ? "Mark as having no office"
                                      : "Mark as having office"
                                  }
                                >
                                  {editingRecord.newHasNoOffice ? (
                                    <BuildingIcon
                              ...
                              {editingRecord.newHasNoOffice && (
                                <p className="text-gray-600 text-xs mt-1">
                                  This person is marked as not having an office
                                </p>
                              )}
                            </div>
                          )}

                          {(missingDataType === "jobTitle" ||
                            missingDataType === "all") && (
                            <div>
                              <label className="block text-sm font-medium text-gray-700 mb-1">
                                Job Title
